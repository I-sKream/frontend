...
const ProductPage = () => {
    ...
    const getChartData = async () => {
        await instance.get(`/api/products/avgprice/${product_id}`)
            .then(res => {
                const date = res.data.map((item) => { return item.date })
                const avg_price = res.data.map((item) => { return item.avg_price })
                setChartdata([date, avg_price])
            })
    }
    ...
    const addCart =  async () => {
        ...
        await instance.post(`/api/products/cart/${product_id}`, {
            product: product_id
        },{
                    ...
                    <div className="col-fixed">
                        <Carousel>
                            <div className='container' style={{ width: `${560 * productList.thumbnail.length}px` }}>
                                {productList.thumbnail.map((item, idx) => {
                                    return (
                                ...
                                <div className='slickarrow' id='prev'>
                                    <button onClick={() => {
                                        if (productImg !== 0) {
                                            ...
                                            removeClass('.click-dot-btn')
                                            document.getElementById(`click-dot${productImg - 1}`).classList.add('active')
                                        }
                                    }}>
                            ...
                            <ul className='clickdot-wrap'>
                                {productList.thumbnail.map((item, idx) => {
                                    return (
                                        <li className='click-dot' key={idx}>
                                            <button
                                                className='click-dot-btn'
                                                id={'click-dot' + idx}
                                                ...
                                                onClick={(e) => {
                                                    ...
                                                    setPorductImg(idx)
                                                    document.querySelector('.container').style.transform = `translate(-${560 * idx}px)`
                                                }}></button>
                                        </li>
                                    )
                                })}
                        ...
                        <div className="detail-wrap">
                                ...
                                <span>
                                    <select name="" className="size-select" onChange={e => setProductSize(e.target.value)}>
                                        {productList.prices.map((item, idx) => {
                                            return (
                                                <option value={idx} key={idx}>{item.size}</option>
                                            )
                                        })}
                                    </select>
                                </span>
                            ...
                            <div className="price-wrap">
                                ...
                                <span className="price-detail">
                                    <div className="price-select">{productList.prices[productSize].price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}원</div>
                                    {productList.prices[productSize].price_diff > 0 ?
                                        <div className="price-diff positive">
                                            <span className='price_diff_icon'>▲</span>
                                            <span>{Math.abs(productList.prices[productSize].price_diff).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}원</span>
                                        </div> :
...
const Carousel = styled.div`
    ...
    .clickdot-wrap {
        ...
        .active { border-top: 2px solid black; }
    }
`
...
const Content = styled.div`
    ...
    .chart-wrap strong { font-size: 18px; }
    .chart-wrap .chart { margin-top: 12px; }
